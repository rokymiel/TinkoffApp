...
class ThemeManager: ThemeManagerProtocol {
    ...
    func apply(theme: Theme) {
        RootAssembly.coreAssembly.themeSaver.writeTheme(theme: theme, fileName: ThemeManager.selectedThemeKey+".txt")
        current = theme
        
    }
    func setTheme() {
        current = RootAssembly.coreAssembly.themeSaver.readTheme(fileName: ThemeManager.selectedThemeKey+".txt")
    }
